...
| 2026/5/18  | v0.0.4 | 新增Task模組之前後端互動細節 | edkiada |
...
* **3.1.1.1 任務基本維護**：系統應 (Shall) 提供使用者新增、編輯與刪除待辦任務之功能。
   - A. 顯示任務(taskList):
      - 前端互動 : 系統會自動發送 `GET` 請求至後端API
      - 後端回應 : 將相應資料格式傳回, 前端在主頁渲染
   - B. 新增任務(Add Task):
      - 前端互動 : 系統提供獨立新增任務頁面(彈出式視窗), 用於搜集任務名稱和參數設定
      - 資料傳輸 : 使用者點擊送出, 前端封裝資料發送 `POST` 請求至後端API.
      - 後端處理 : 後端接受請求資料後, 驗證格式並將資料寫入資料庫, 後通知前端重新渲染任務列表.
   - C. 刪除任務(Delete Task):
      - 前端互動 : 左滑單項任務並顯示刪除按鈕.
      - 資料傳輸 : 使用者點擊刪除, 前端攜帶任務ID並發送 `DELETE` 請求至後端API.
      - 後端處理 : 後端接受刪除請求, 並通知前端重新渲染任務列表. 
